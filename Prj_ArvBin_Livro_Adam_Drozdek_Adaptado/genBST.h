...
public:
    ...
    void balance(T*,int,int); //Veja que este método para ser invocado precisaria dos parâmetros de entrada criados previamente... Desafio...
    void displayTree(); //Feito por Irineu - no fim deste arquivo
protected:
    BSTNode<T>* root;
...
//Veja que este método para ser invocado precisaria dos parâmetros de entrada criados previamente... Desafio...
template<class T>
...
//Feito por Irineu: Desenha a árvore para exibição de nós-dados contidos
template<class T>
void BST<T>::displayTree() { //baseado em displayTree, p.371 - Livro Robert Lafore, ED e Alg em Java - Ciência Moderna, 2004.
	BSTNode<T>* a = this->root; //inicia do root
	stack<BSTNode<T>*> globalStack; //uso de bib stack.h da STD lib (contâiner C++ LIFO)
	globalStack.push(a);
	int nBlanks = 32;
	...
	while (isRowEmpty == false){
		...
		while (globalStack.empty()== false){
			BSTNode<T>* temp = globalStack.top(); //pega topo da pilha global e...
